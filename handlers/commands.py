...
def setup_commands(client, db: Database, user_states: dict, user_client=None):
    ...
    async def cmd_start(event):
        if event.sender_id not in OWNER_IDS:
            return
        menu_keyboard = [
            [Button.text("📥 Источники", resize=True), Button.text("📤 Склады", resize=True)],
            [Button.text("📋 Список", resize=True), Button.text("⚙️ Настройки", resize=True)],
        ]
        await event.respond(
            "Бот-репостер готов.\n\nКоманды:\n"
    ...
    async def cmd_settings(event):
        ...
        buttons = [
            [Button.inline(f"Шаг 1 (все посты)", b"set_step_1")],
            [Button.inline(f"Шаг 2 (каждый 2-й)", b"set_step_2")],
            [Button.inline(f"Шаг 3 (каждый 3-й)", b"set_step_3")],
            [Button.inline(f"Шаг 4 (каждый 4-й)", b"set_step_4")],
        ]
        await event.respond(
            f"<b>Шаг репоста</b>\n\n"
            f"Сейчас: <b>{step}</b>\n\n"
            f"1 — всё подряд\n"
            f"2 — каждый второй пост\n"
            f"3 — каждый третий\n"
            f"Выбери:",
            parse_mode='html',
            buttons=buttons
    ...
    # Обработка добавления источников/складов и кнопки Настройки
    ...
    async def private_steps(event):
        ...
        # Кнопки из reply-меню
        if text == "⚙️ Настройки" or text.lower() == "настройки":
            step = db.get_repost_step()
            buttons = [
                [Button.inline(f"Шаг 1 (все посты)", b"set_step_1")],
                [Button.inline(f"Шаг 2 (каждый 2-й)", b"set_step_2")],
